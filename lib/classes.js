...
class Build {
  ...
  async buildLists(build) {
    var skillCodes = build.skills.map(skillCode => {
      return skillMap[skillCode];
    });
    const URL = `https://worldofwarships.com/en/content/captains-skills/?skills=${skillCodes}&ship=${this.ship.type}`;
    this.captSkillsUrl = !isEmpty(skillCodes) ? `${URL}\n` : "";
  }
}
