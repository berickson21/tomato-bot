...
const { filter, startCase } = require("lodash");
...
async function buildCaptSkillsEmbed(shipName) {
  ...
  if (skillString) {
    if (
      skillString === "Soon™" ||
      (shipBuild.note && shipBuild.note.includes("pre-0.10.0"))
    ) {
      messageCounts.emptySkills[shipBuild.name] =
        (messageCounts.emptySkills[shipBuild.name] || 0) + 1;
    } else delete messageCounts.emptySkills[startCase(shipName)];
...
async function buildUpgradesEmbed(shipName) {
  ...
  if (upgradeString) {
    ...
    } else delete messageCounts.emptyUpgrades[startCase(shipName)];
...
function buildRequestCountMessage() {
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .addFields(
      {
        name: "Ships without captain skills",
        value: topEmptySkills.join("\n")
      },
